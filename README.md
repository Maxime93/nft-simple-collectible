...
## Simple Collectible
### Testing on my local ganache blockchain
If not done before:
`brownie networks add development local host=HTTP://127.0.0.1:7545 cmd=ganache` Which creates connection to local blockchain
`brownie console --network local` Which install dependencies found in the repo and starts console.

Exit console. Edit `scripts/simple_collectible/deploy_simple.py`:
```
#!/usr/bin/python3
import os
...
Last uncommented creates contract from the local ganache blockchain at index 1.
Then we can `browznie console` and do the following:
```
SimpleCollectible.at('0x3FB85BD0347bD234ca23c8DA903a21D946aED780').createCollectible("None",{"from": accounts[0]}) # calls the createCollectible of the smart contract with a tokenURI of "None".
SimpleCollectible.at('0x3FB85BD0347bD234ca23c8DA903a21D946aED780').ownerOf(0, {"from": accounts[0]}) # Returns the owner of the contract (Effectively NFT)
SimpleCollectible.at('0x3FB85BD0347bD234ca23c8DA903a21D946aED780').ownerOf(0, {"from": accounts[1]})  # Returns the same owner of the contract (called by a different account).
SimpleCollectible.at('0x3FB85BD0347bD234ca23c8DA903a21D946aED780').createCollectible("None", {"from": accounts[4]}) # Create another contract (NFT) from account index 4: 0x617F2B1927742f8f7b9590Ce58FaF0e5dEDF8628
SimpleCollectible.at('0x3FB85BD0347bD234ca23c8DA903a21D946aED780').ownerOf(1, {"from": accounts[6]}) # Returns account[4] address. Seems like the the simple collectibles are indexed
```


# nft-mix (Original README from https://github.com/PatrickAlphaC/nft-mix)
